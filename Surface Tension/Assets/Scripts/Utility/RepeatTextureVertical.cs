...
public class RepeatTextureVertical : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        for (int i = 1, l = (int)Mathf.Round(floor.bounds.size.y); i < l; i++)
        {

            child = Instantiate(childPrefab) as GameObject;
            child.transform.position = (transform.position - (new Vector3(spriteSize.y - (spriteSize.y / 2), 0, 0) * i));
            child.transform.parent = transform;

            child2 = Instantiate(childPrefab) as GameObject;
            child2.transform.position = (transform.position - (new Vector3(-spriteSize.y + (spriteSize.y / 2), 0, 0) * i));
